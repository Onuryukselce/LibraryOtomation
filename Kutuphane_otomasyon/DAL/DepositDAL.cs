...
namespace Kutuphane_otomasyon.DAL
{
    class DepositDAL
    {
        ...
        public bool INSERT(Deposit data) //ICrud interface'de tanımlanan polymorph INSERT metodu implemente edilmiştir.
        {
            bool status = false; //Sonucun başarılı olup olmadığına dair veriyi tutacak bool değişken

            OleDbCommand command = new OleDbCommand();
            command.CommandType = CommandType.Text;
            command.CommandText = "INSERT INTO Deposit(MemberNumber, BookNo, DepositDate, WithdrawDate, PenaltyMoney,Status) VALUES ( ?, ?, ?, ?, ?, ?)";
            command.Parameters.AddWithValue("@MemberNumber", data.MemberNo);
            command.Parameters.AddWithValue("@BookNo", data.BookNo);
            command.Parameters.AddWithValue("@DepositDate", Convert.ToDateTime(data.DepositDate));
            command.Parameters.AddWithValue("@WithdrawDate", Convert.ToDateTime(data.WithdrawDate));
            command.Parameters.AddWithValue("@PenaltyMoney", Convert.ToDecimal(data.Penalty));
        ...
        public bool UPDATE(Deposit data) //ICrud interface'de tanımlanan polymorph UPDATE metodu implemente edilmiştir.
        {
            bool status = false; //Sonucun başarılı olup olmadığına dair veriyi tutacak bool değişken
            ...
            OleDbCommand command = new OleDbCommand();
            command.CommandType = CommandType.Text;
            command.CommandText = "UPDATE Deposit SET MemberNumber = ?, BookNo = ?, DepositDate = ?, WithdrawDate = ?, PenaltyMoney = ?, Status = ? Where ID = ?";
            command.Parameters.AddWithValue("@MemberNumber", data.MemberNo);
            command.Parameters.AddWithValue("@BookNo", data.BookNo);
            command.Parameters.AddWithValue("@DepositDate", Convert.ToDateTime(data.DepositDate));
            command.Parameters.AddWithValue("@WithdrawDate", Convert.ToDateTime(data.WithdrawDate));
        ...
        public bool DELETE(Deposit data) //ICrud interface'de tanımlanan polymorph DELETE metodu implemente edilmiştir.
        {
            bool status = false;

            OleDbCommand command = new OleDbCommand();
            command.CommandType = CommandType.Text;
            command.CommandText = "DELETE FROM Deposit Where ID = ? ";
            command.Parameters.AddWithValue("@ID", long.Parse(data.ID));
            try // Sorgu execute sırasında, sorguda ya da bağlantıda bir hata meydana gelmesi durumuna karşı hata ayıklama yapılıyor.
            {
                int resultSet = ExecuteNonQuery(command); //ExeCuteNonQuery fonksiyonu etkilenen kolon sayısını döndürdüğü için resultSet altında bunu tutuyor, aynı zamanda bu satırda sorgu çalışıyor.
                if (resultSet != 0) //Eğer resultSet 0 değilse, işlemin hatasız meydana geldiğini bildiren status = true, eğer hi sonuç etkilenmemişse bir problem olduğunu belirten status = false
